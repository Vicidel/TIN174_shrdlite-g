...
export function parseUtteranceIntoPlan(world : World, utterance : string) : string[] {
    ...
    try {
        ...
        var plan_cost : number = Number.POSITIVE_INFINITY;

        for(var i=0 ; i<plans.length ; i++){
            if(plans[i].plan.length<plan_cost){
                plan_cost= plans[i].plan.length;
                var finalPlan : string[] = plans[i].plan;
            }
        }
...
export function splitStringIntoPlan(planstring : string) : string[] {
    ...
    var actions : {[act:string] : string}
        = {p:"Picking", d:"Dropping", l:"Going left", r:"Going right", b:"Going backward", f:"Going forward"};
